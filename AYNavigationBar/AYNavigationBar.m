...
typedef void (^_AYViewControllerWillAppearBlock)(UIViewController *viewController, BOOL animated);
...
- (void)ay__viewWillAppear:(BOOL)animated
{
    ...
    !self.ay_viewWillAppearBlock ?: self.ay_viewWillAppearBlock(self, animated);
...
- (void)registerNavigationBar
{
    ...
    self.ay_prefersNavigationBarHidden = !self.navigationController.ay_navigationBarHidden;
...
- (BOOL)ay_prefersNavigationBarHidden
{
    return [objc_getAssociatedObject(self, _cmd) boolValue];
}

- (void)setAy_prefersNavigationBarHidden:(BOOL)ay_prefersNavigationBarHidden
{
    objc_setAssociatedObject(self, @selector(ay_prefersNavigationBarHidden), @(ay_prefersNavigationBarHidden), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (_AYViewControllerWillAppearBlock)ay_viewWillAppearBlock
{
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setAy_viewWillAppearBlock:(_AYViewControllerWillAppearBlock)ay_viewWillAppearBlock
{
    objc_setAssociatedObject(self, @selector(ay_viewWillAppearBlock), ay_viewWillAppearBlock, OBJC_ASSOCIATION_COPY_NONATOMIC);
}
...
+ (void)load
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSArray *sels = @[@"setNavigationBarHidden:animated:", @"pushViewController:animated:"];
        [sels enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            Method originalMethod = class_getInstanceMethod(self, NSSelectorFromString(obj));
            NSString *swizzledSel = [@"ay__" stringByAppendingString:obj];
            Method swizzledMethod = class_getInstanceMethod(self, NSSelectorFromString(swizzledSel));
...
- (void)ay__pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    [self ay_setupViewControllerBasedNavigationBarAppearanceIfNeeded:viewController];
    
    [self ay__pushViewController:viewController animated:animated];
}

- (void)ay_setupViewControllerBasedNavigationBarAppearanceIfNeeded:(UIViewController *)appearingViewController
{
    if (self.ay_basedNavigationBarAppearanceDisabled) {
        return;
    }
    
    __weak typeof(self) weakSelf = self;
    _AYViewControllerWillAppearBlock block = ^(UIViewController *viewController, BOOL animated) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (strongSelf) {
            [strongSelf setNavigationBarHidden:viewController.ay_prefersNavigationBarHidden animated:animated];
        }
    };
    
    appearingViewController.ay_viewWillAppearBlock = block;
    
    UIViewController *disappearingViewController = self.topViewController;
    if (disappearingViewController && !disappearingViewController.ay_viewWillAppearBlock) {
        disappearingViewController.ay_viewWillAppearBlock = block;
    }
}
...
- (void)setAy_navigationBarHidden:(BOOL)ay_navigationBarHidden
{
    ...
    self.topViewController.ay_prefersNavigationBarHidden = !ay_navigationBarHidden;
}
...
- (BOOL)ay_basedNavigationBarAppearanceDisabled
{
    return [objc_getAssociatedObject(self, _cmd) boolValue];
}

- (void)setAy_basedNavigationBarAppearanceDisabled:(BOOL)ay_basedNavigationBarAppearanceDisabled
{
    objc_setAssociatedObject(self, @selector(ay_basedNavigationBarAppearanceDisabled), @(ay_basedNavigationBarAppearanceDisabled), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
